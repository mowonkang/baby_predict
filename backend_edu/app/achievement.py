...
_EBS = EduOption(name="EBS (초등·중학·고교)", provider="EBS", cost="무료~저렴",
                 note="학년·과목별 강의(일부 프리미엄만 유료)", url="https://www.ebs.co.kr/")
_ELEARN = EduOption(name="e학습터", provider="교육부·에듀넷", cost="무료",
                    note="초1~중3 교과 영상·평가", url="https://cls.edunet.net/")
_NEULBAEUM = EduOption(name="늘배움(평생학습포털)", provider="국가평생교육진흥원", cost="무료",
                       note="무료 강좌 모음", url="https://www.lifelongedu.go.kr/")
_LIB = EduOption(name="공공도서관 프로그램", provider="지역 도서관", cost="무료/저렴",
                 note="독서·학습 지원(거주지 도서관 검색)", url="")
_AISARANG = EduOption(name="아이사랑(임신육아종합포털)", provider="보건복지부", cost="무료",
                      note="영유아 발달·돌보기·검진 정보", url="https://www.childcare.go.kr/")

_FREE_GENERAL: list[EduOption] = [_EBS, _ELEARN, _NEULBAEUM, _LIB]

_FREE_BY_SUBJECT: dict[str, list[EduOption]] = {
    "수학": [
        EduOption(name="똑똑! 수학탐험대", provider="교육부", cost="무료", note="초등 AI 수학(공교육)", url="https://www.toctocmath.kr/"),
        EduOption(name="칸아카데미 한국어", provider="Khan Academy", cost="무료", note="개념 영상 + 연습", url="https://ko.khanacademy.org/"),
        EduOption(name="EBS MATH", provider="EBS", cost="무료", note="수학 전문", url="https://www.ebsmath.co.kr/"),
    ],
    "영어": [
        EduOption(name="EBSe", provider="EBS", cost="무료", note="영어 전문 무료 채널", url="https://www.ebse.co.kr/"),
        EduOption(name="칸아카데미", provider="Khan Academy", cost="무료", note="문법·읽기", url="https://ko.khanacademy.org/"),
    ],
    "국어": [EduOption(name="EBS 국어 + 도서관 독서", provider="EBS·도서관", cost="무료", note="독해·어휘는 다독이 기본", url="https://www.ebs.co.kr/")],
    "과학": [EduOption(name="EBS 과학·KOCW 공개강의", provider="EBS·KOCW", cost="무료", note="개념 영상", url="http://www.kocw.net/")],
    "사회": [EduOption(name="EBS 사회·KOCW 공개강의", provider="EBS·KOCW", cost="무료", note="개념 영상", url="http://www.kocw.net/")],
    # 유아·영아(비교과)
    "한글": [EduOption(name="토도한글", provider="에누마", cost="저렴(무료체험)", note="유아~초등 한글 앱", url="https://todoschool.com/"),
             _ELEARN],
    "기초수": [EduOption(name="토도수학", provider="에누마", cost="저렴(무료체험)", note="유아~초등 수학 앱", url="https://todoschool.com/"),
              EduOption(name="똑똑! 수학탐험대", provider="교육부", cost="무료", note="초등 AI 수학", url="https://www.toctocmath.kr/")],
    "독서": [_LIB, _AISARANG],
...
def _free_for(subject: str) -> list[EduOption]:
    ...
    if not opts:           # 매핑 없으면 일반 공공자원
        opts = [_EBS, _ELEARN]
    elif _EBS not in opts and _AISARANG not in opts:
        opts.append(_EBS)  # 교과 과목엔 EBS 항상 보강
    return opts[:3]
